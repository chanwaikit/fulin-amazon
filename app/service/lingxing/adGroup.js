      ...
      for (let j = 0; j < resultArray.length; j++) {
        ...
        if (sbResult.data.total > 0) { // 广告分组里头有活动的，才列为SB或者SBV的广告组合
          ...
          if (sbResult.data.list[0].ad_format === 'video') {
            // sbv组合
            obj.type = 'sbv';
            const profitObj = await ctx.model.Fulin.SbvGroup.findByPk(obj.portfolio_id);
            if (!profitObj) {
              await ctx.model.Fulin.SbvGroup.create(obj);
            }
          ...
          const profitObj = await ctx.model.Fulin.AdGroup.findByPk(obj.portfolio_id);

          if (!profitObj) {
            await ctx.model.Fulin.AdGroup.create(obj);
          }
          pResult.push(obj);
        }
      }
